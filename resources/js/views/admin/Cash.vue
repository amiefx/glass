<template>
  ...
  dense
    ...
    :server-items-length="cashes.total"
  ...
  >
    <template v-slot:top>
      <v-toolbar flat color="">
        <v-toolbar-title class="text-primary"> {{ $t('Cash Register') }}</v-toolbar-title>
        <v-divider
          class="mx-4"
          inset
          vertical
        ></v-divider>
        <v-spacer></v-spacer>
        <v-toolbar-title class="bg-success"> Balance: {{ balance }}</v-toolbar-title>
...
<script>
  export default {
    ...
    methods: {
      updateStatus(item) {
          const index = this.cashes.data.indexOf(item);
          axios.post('/api/change-status', {'status': item.is_active, 'cash': item.id})
            .then(res => {
               this.text = res.data.cash.name + "'s Status Updated to " + res.data.cash.is_active
               this.snackbar = true
                })
            .catch(error => {
                // this.text = error.response.cash.name + "'s Status Cannot be Updated" + error.response.cash.status
                this.cashes.data[index].is_active = error.response.data.cash.is_active
                this.snackbar = true
      ...
      searchIt(e){
          ...
          if(e.length <= 0){
            ...
            const orderBy = this.options.sortDesc.length > 0 || this.options.sortDesc[0] ? 'asc' : 'desc';
                axios.get(`/api/cashes?page=${e.page}`,{params:{'per_page': e.itemsPerPage, 'sort_by': sortBy, 'order_by': orderBy}})
                .then(res => {
      ...
      deleteItem (item) {
        ...
        if(decide){
            axios.delete('/api/cashes/'+item.id)
            .then(res => {
                this.snackbar = true
                this.cashes.data.splice(index, 1)
            })
      ...
      save () {
        if (this.editedIndex > -1) {
          ...
          axios.put('/api/cashes/'+this.editedItem.id, this.editedItem)
        //  .then(res => Object.assign(this.cashes[this.editedIndex], this.editedItem))
         .then(res => {
           console.log(res)
             this.text = "Record Updated Successfully!";
             this.snackbar = true;
             Object.assign(this.cashes.data[index], res.data.cash)
         })
         .catch(err => {
             console.log(err.response)
        ...
        //  Object.assign(this.cashes.data[this.editedIndex], this.editedItem)
            ...
            axios.post('/api/cashes', this.editedItem)
            .then(res => {
                this.text = "Record Added Successfully!";
                this.snackbar = true;
                this.cashes.data.push(res.data.cash)
            })
            .catch(err => {
                console.dir(err)

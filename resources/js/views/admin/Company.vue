              ...
              <v-container>
                <v-row>
                  ...
                  <v-col cols="12" md="6" lg="6" >
                    ...
                    <v-text-field v-model="editedItem.city" label="City" :rules="[rules.required]"></v-text-field>
                    <v-text-field v-model="editedItem.country" label="Country" :rules="[rules.required]"></v-text-field>
  ...
  export default {
    ...
    methods: {
      updateStatus(item) {
          ...
          axios.post('/api/change-status', {'status': item.is_active, 'business': item.id})
            .then(res => {
               this.text = res.data.business.name + "'s Status Updated to " + res.data.business.is_active
               this.snackbar = true
                })
            .catch(error => {
                // this.text = error.response.business.name + "'s Status Cannot be Updated" + error.response.business.status
                this.business.data[index].is_active = error.response.data.business.is_active
                this.snackbar = true
      ...
      searchIt(e){
          ...
          if(e.length <= 0){
            ...
            const orderBy = this.options.sortDesc.length > 0 || this.options.sortDesc[0] ? 'asc' : 'desc';
                axios.get(`/api/business?page=${e.page}`,{params:{'per_page': e.itemsPerPage, 'sort_by': sortBy, 'order_by': orderBy}})
                .then(res => {
      ...
      deleteItem (item) {
        ...
        if(decide){
            axios.delete('/api/business/'+item.id)
            .then(res => {
                this.snackbar = true
                this.business.data.splice(index, 1)
            })
      ...
      save () {
        if (this.editedIndex > -1) {
          ...
          axios.put('/api/business/'+this.editedItem.id, this.editedItem)
        ...
        //  Object.assign(this.business.data[this.editedIndex], this.editedItem)
            ...
            axios.post('/api/business', this.editedItem)
            .then(res => {

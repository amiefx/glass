...
<script>
  export default {
    ...
    methods: {
      updateStatus(item) {
          const index = this.brands.data.indexOf(item);
          axios.post('/api/change-status', {'status': item.is_active, 'brand': item.id})
            .then(res => {
               this.text = res.data.brand.name + "'s Status Updated to " + res.data.brand.is_active
               this.snackbar = true
                })
            .catch(error => {
                // this.text = error.response.brand.name + "'s Status Cannot be Updated" + error.response.brand.status
                this.brands.data[index].is_active = error.response.data.brand.is_active
                this.snackbar = true
      ...
      searchIt(e){
          ...
          if(e.length <= 0){
            ...
            const orderBy = this.options.sortDesc.length > 0 || this.options.sortDesc[0] ? 'asc' : 'desc';
                axios.get(`/api/brands?page=${e.page}`,{params:{'per_page': e.itemsPerPage, 'sort_by': sortBy, 'order_by': orderBy}})
                .then(res => {
      ...
      deleteItem (item) {
        ...
        if(decide){
            axios.delete('/api/brands/'+item.id)
            .then(res => {
                this.snackbar = true
                this.brands.data.splice(index, 1)
            })
      ...
      save () {
        if (this.editedIndex > -1) {
          ...
          axios.put('/api/brands/'+this.editedItem.id, this.editedItem)
        //  .then(res => Object.assign(this.brands[this.editedIndex], this.editedItem))
         .then(res => {
           console.log(res)
             this.text = "Record Updated Successfully!";
             this.snackbar = true;
             Object.assign(this.brands.data[index], res.data.brand)
         })
         .catch(err => {
             console.log(err.response)
        ...
        //  Object.assign(this.brands.data[this.editedIndex], this.editedItem)
            ...
            axios.post('/api/brands', this.editedItem)
            .then(res => {
                this.text = "Record Added Successfully!";
                this.snackbar = true;
                this.brands.data.push(res.data.brand)
            })
            .catch(err => {
                console.dir(err)

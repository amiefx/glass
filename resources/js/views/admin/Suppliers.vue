...
<script>
  export default {
    ...
    methods: {
      updateStatus(item) {
          const index = this.suppliers.data.indexOf(item);
          axios.post('/api/change-status', {'status': item.is_active, 'supplier': item.id})
            .then(res => {
               this.text = res.data.supplier.name + "'s Status Updated to " + res.data.supplier.is_active
               this.snackbar = true
                })
            .catch(error => {
                // this.text = error.response.supplier.name + "'s Status Cannot be Updated" + error.response.supplier.status
                this.suppliers.data[index].is_active = error.response.data.supplier.is_active
                this.snackbar = true
      ...
      searchIt(e){
          ...
          if(e.length <= 0){
            ...
            const orderBy = this.options.sortDesc.length > 0 || this.options.sortDesc[0] ? 'asc' : 'desc';
                axios.get(`/api/suppliers?page=${e.page}`,{params:{'per_page': e.itemsPerPage, 'sort_by': sortBy, 'order_by': orderBy}})
                .then(res => {
      ...
      save () {
        ...
        //  .then(res => Object.assign(this.suppliers[this.editedIndex], this.editedItem))
         .then(res => {
           console.log(res)
             this.text = "Record Updated Successfully!";
             this.snackbar = true;
             Object.assign(this.suppliers.data[index], res.data.supplier)
         })
         .catch(err => {
             console.log(err.response)
        ...
        //  Object.assign(this.suppliers.data[this.editedIndex], this.editedItem)
            ...
            .then(res => {
                ...
                this.snackbar = true;
                this.suppliers.data.push(res.data.supplier)
            })
            .catch(err => {
                console.dir(err)

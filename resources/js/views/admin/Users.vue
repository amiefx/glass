<template>
  ...
  >
                ...
                <v-row>
                  <v-col cols="12">
                   ...
                   <v-text-field
                    ...
                    <v-text-field v-model="editedItem.contact_no" label="Contact No" :rules="[rules.required]"></v-text-field>
                    <v-select v-model="editedItem.type" :items="roles" label="Select Role" :rules="[rules.required]"></v-select>
                  </v-col>
                </v-row>
              </v-container>
            </v-card-text>
    ...
    <template v-slot:item.type = "{ item }">
        <v-edit-dialog large block persistent :return-value.sync="item.type" @save="updateRole(item)">
            {{item.type}}
            <template v-slot:input>
                <v-select v-model="item.type" :items="roles" label="Select Role" :rules="[rules.required]"></v-select>
            </template>
        </v-edit-dialog>
    </template>
  ...
  export default {
    ...
    methods: {
      updateRole(item) {
          ...
          axios.post('/api/change-role', {'role': item.type, 'user': item.id})
            .then(res => {
               // this.users.data[index].role = res.data.data.user.role
               this.text = res.data.user.name + "'s Role Updated to " + res.data.user.type
               this.snackbar = true
                })
            .catch(error => {
                // this.text = error.response.data.user.name + "'s Role Cannot be Updated" + error.response.data.user.role
                this.users[index].type = error.response.user.type
                this.snackbar = true
      ...
      updateStatus(item) {
          ...
          axios.post('/api/change-status', {'status': item.is_active, 'user': item.id})
            ...
            .catch(error => {
                ...
                this.users.data[index].type = error.response.data.user.type
                this.snackbar = true
                console.dir(error.response)
                })
      },
